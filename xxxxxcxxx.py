...
if (x.strftime("%x"))>(g.strftime("%x")):
 print('\n\n')
 print("    "+ 'WARING⛔ THE TOOL CODEING BY MRBX CODER 404\nTOOL EXPIERE BRO⛔\nBO KRINI TOOL NAMA BNERA BO @MRBX_709')
 print('\n\n')
 print(x)
...
def su():
	...
	print(f'''
	\033[1;41m\033[1;97mTOOL MRBX\033[1;0m
...
def crackmbasic_MRBX(idf,pwv,nmf):
	global loop,ok,cp
	sys.stdout.write(f"\r[ [VIP] [MRBX]{P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}--{ok}-{P}]—{P}[{k}--{cp}-{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
